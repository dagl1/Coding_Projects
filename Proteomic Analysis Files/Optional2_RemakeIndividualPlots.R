...
### This file allows you to remake volcano and stoichiometry plots, it will
### completely redo the analysis and create plots with the name "remade".
### Use this function if you want to remake your plots with different complexes
### highlighted in the "custom" plots. Or if you want to change any other of the
### inputs (such as changing from pooledAGAR to individual AGAR,  changing
### Mendoza imputation, or setting thresholds).
###
### Specify in which run you are changing these plots by setting NameOfRun,
### then provide the name as it was given in the excel file (it will read the
### file in the run map, which was copied from the excel file used by Main1, 
### if you change the actual excel file, this script will not take that into
### account.
###
### Note that this does not replace any of the saved files used for clustering!!
### You can NOT individually change the larger dataset without rerunning Main1_analyseData
### [this might be on the todo list if it turns out anyone needs this,
### but I currently do not see how  plots created with different settings can or
### should be compared].
...
pathToFolderWithFiles = ("E:/PhD/AllMassspecdata/H_Clust Project/HIFBAPMID/") 
...
NameOfRun = "PoolMenImMeIBA7"   
          # "Run1AllSamples"

### Here you provide the names of the sample you would like to change
### Make sure to type it correctly, as and to separate by , and do not forget ""
### around each name
NameOfSamples = c("MIDEAS S1 WT C-MT Nuc HEK293 File 51")
              # c("MIDEAS S1 WT C-MT Nuc HEK293 File 51","DNTTIP1 WT C-GFP Nuc HEK293 File 12")
...
CreateASingleAGARMatrixLFQ  = F  # If this is TRUE, all agar samples are pooled together
                                    ...
                                    # in one or more control samples. This only appears to happen with so far with
...
UseMendozaImputation = T         # See Mendoza imputation, increases contrast between high and low values 
...
MakeCurrentMatrixReadyForPlotsAndPlot <- function(currentMatrixContainingSamples,iterator2){
  ...
  createVolcanoPlots <- function(currentMatrixContainingSamples){
    ...
    outputName = paste("Remade_", nameOfSample," Custom Volcano", ".png", sep = "")
    ...
    ### Save plot
    outputName = paste("Remade_", nameOfSample," C Volcano plot", ".png", sep = "")
    ...
    outputName = paste("Remade_", nameOfSample," C Volcano plot", ".pdf", sep = "")
    ...
    outputName = paste("Remade_", nameOfSample," Complexes Volcano plot", ".png", sep = "")
    ...
    ### Save plot [todo] temp for checking with pdf
    outputName = paste("Remade_", nameOfSample," Complexes Volcano plot", ".pdf", sep = "")
    pdf(outputName ,width = 8, height = 8)
    print(pAllComplexes)
    dev.off() 
    ######################################################################
    ...
    ### Set name of plot
    outputName = paste("Remade_", nameOfSample," Standard Volcano plot", ".png", sep = "")
    png(outputName ,width = 8, height = 8, 
        units = "in", res = 1800)   
    print(pStandard)
    dev.off() 
    ...
    ### Save plot [todo] temp for checking with pdf
    outputName = paste("Remade_", nameOfSample," Standard Volcano plot", ".pdf", sep = "")
    pdf(outputName ,width = 8, height = 8)
    print(pStandard)
    dev.off() 
    ######################################################################
  ...
  if(length(stochiometryDataFrame$Stoichiometry)>2){
    ...
    for(stoichiometryIterator in 1:length(amountOfHitsInStoichiometryPlot))  {
      ...
      if(length(stochiometryDataFrameToUse$Stoichiometry)>2){
        ...
        #####################################
        outputName = paste("Remade_", nameOfSample," ",paste("Stoichiometry_",as.character(amountOfHitsInStoichiometryPlot[stoichiometryIterator]),sep = ""), ".png", sep = "")
        setwd(outputLocationForStoichiometry)
        
        png(outputName,unit="in",res=600,height=8,width=8)
        currentGraphTitle1 = paste("Remade_",nameOfSample, " Stochiometry", sep = "")
        ...
        #####################################
        outputName = paste("Remade_", nameOfSample," ",paste("Custom Stoichiometry_",as.character(amountOfHitsInStoichiometryPlot[stoichiometryIterator]),sep = ""), ".png", sep = "")
        setwd(outputLocationForSpecifiedStoichiometryPlots)
        
        png(outputName,unit="in",res=600,height=8,width=8)
        currentGraphTitle1 = paste("Remade_",nameOfSample, "Custom Stochiometry", sep = "")
        ...
        outputName = paste( "Remade_",nameOfSample," ",paste("Complexes Stoichiometry_",as.character(amountOfHitsInStoichiometryPlot[stoichiometryIterator]),sep = ""), ".png", sep = "")
        setwd(outputLocationForComplexStoichiometryPlots)
        
        png(outputName,unit="in",res=600,height=8,width=8)
        currentGraphTitle1 = paste("Remade_",nameOfSample, "Complexes Stochiometry", sep = "")
...
for (iterator1 in 1:length(matrixContainingAllProteinGroups)){
  ...
  if(length(IndicesInExperimentalOfValuesInAgar)>0){ #make sure unused proteingroups in directory dont cause problems
    ...
    for(iterator2 in  1:length(IndicesInExperimentalOfValuesInAgar)){ #get columns for analysis
      ...
      ### Added in comparison to Main1, this only reruns the data with the specified sample to create new plots
      ### These have the name "remade" added to them.
      
      if(agarSamplesInFile$Name[iterator2] %in% NameOfSamples){
        ...
        if(OverWritePerSampleClustering == TRUE||( OverWritePerSampleClustering == FALSE
                                                   & file.exists(paste0(outputLocationForControlVsExperimentClustering,"/",nameOfSample,"_BeforeImputation_", currentRun1,".png")))){
          ...
          png(paste("Remade_",nameOfSample,"_BeforeImputation_", currentRun1,".png",sep=""),unit="in",res=1800,height=8,width=8)
          draw(Heatmap(mat))
          dev.off()
        }
        ...
        if(OverWritePerSampleClustering == TRUE||( OverWritePerSampleClustering == FALSE 
                                                   & file.exists(paste0(outputLocationForControlVsExperimentClustering,"/",nameOfSample,"_AfterImputation_", currentRun1,".png")))){
          ...
          ### Set directory and save heatmap
          setwd(outputLocationForControlVsExperimentClustering)
          png(paste("Remade_",nameOfSample,"_AfterImputation_", currentRun1,".png",sep=""),unit="in",res=1800,height=8,width=8)
          draw(Heatmap(mat))
          dev.off()
        }
        ...
        setwd(outputLocationForIndividualMatricesDataFrames)
        saveRDS(currentMatrixContainingSamples,paste("Remade_",nameOfSample,"MatrixAfterImputationAndAnalysis.rds"))
      }
